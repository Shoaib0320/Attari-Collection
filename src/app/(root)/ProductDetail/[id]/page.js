...
import { useSession } from "next-auth/react";
import { toast } from "react-hot-toast";
import { addCarts, addToCart } from "@/actions/addToCartAction";
...
export default function ProductDetail({ params }) {
  ...
  const handleAddToCart = async () => {
    const cartItem = {
        ...
        user: product.userId, // Make sure to get the userId correctly
    };
  ...
  const handleDecrement = () => quantity > 1 && setQuantity((prevQuantity) => prevQuantity - 1);
