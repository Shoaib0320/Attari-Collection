...
//   // Fetch session using the auth function
//   useEffect(() => {
//     const fetchSession = async () => {
//       const sessionData = await auth();
//       setSession(sessionData); // Set the session data
//     };
...
//       return;
//     }
//     // Add to cart logic with user session
//     alert("Added to cart!");
//   };
...
export default async function ProductDetail({ params }) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch("/api/cart", {
        ...
        body: JSON.stringify({
          user: session.user, // Include user data from session
          product,
          quantity,
        }),
      });
  ...
  if (loading) {
    return (
      <section className="text-gray-600 body-font">
        {/* Loading skeleton */}
        {/* (Your loading UI code remains the same) */}
      </section>
    );
  }
