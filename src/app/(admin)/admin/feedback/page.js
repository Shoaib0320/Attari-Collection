...
//       prev.map((col) => ({
//         ...col,
//         hidden: !show, // If `show` is true, make all columns visible, otherwise hide them
//       }))
//     )
//   }
...
//                 onClick={() => toggleColumn(col.id || col.accessorKey)}
//                 className="flex items-center justify-between"
//               >
//                 {col.header}
//                 {!col.hidden && <FaCheck className="ml-2 text-green-500" />}
//               </DropdownMenuItem>
//             ))}
//           </DropdownMenuContent>
//         </DropdownMenu>
...
//       {/* Data Table */}
//       <DataTable
//         columns={columns.filter((col) => !col.hidden)} // Filter out hidden columns
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <div className="mb-4">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline">Columns <FaCaretDown />
            </Button>
          </DropdownMenuTrigger>
          ...
          <DropdownMenuContent>
            ...
            {columns.map((col) => (
              <DropdownMenuItem
                ...
                onClick={() => toggleColumn(col.id || col.accessorKey)}
                className={`flex items-center justify-between ${
                  !col.hidden ? "bg-gray-100 text-gray-600 font-semibold" : ""
                }`}
              >
                {col.hidden ? `Show ${col.header}` : `${col.header}`}
                {/* Only show FaCheck for visible columns */}
                {!col.hidden && <FaCheck className="ml-2 text-green-500" />}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
